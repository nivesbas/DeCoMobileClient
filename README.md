...
`app.json` references the file at the root and `expo prebuild` copies it into `android/app/google-services.json`. If you edit the root file, re-run prebuild so the copy inside `android/` is in sync.
...
### 4. Build a standalone APK (side-load testing)

```bash
cd android && ./gradlew assembleRelease
# APK at android/app/build/outputs/apk/release/app-release.apk (~5–7 min, longer on first run)
```

The release variant is signed with the Expo-generated `android/app/debug.keystore` (checked into the repo) — fine for internal side-load testing. Do NOT ship this APK to Play Store; the Play Store variant needs a real upload key and is out of scope for now (we're not publishing yet).

If you've edited `app.json`, `google-services.json`, or added a new native plugin, refresh the generated project first:

```bash
npx expo prebuild --platform android --clean
```

`--clean` wipes `android/` and regenerates from `app.json`. The debug keystore is regenerated too, so the APK signature changes — users with the old APK installed will need to uninstall before installing the new one.

### 5. Fast JS-only rebuild

If the only thing you changed is TypeScript / JSX (no native module, no `app.json` edit), skip prebuild:

```bash
cd android && ./gradlew assembleRelease
```

Gradle re-bundles the JS via Metro during the build and produces a new APK in ~60–90 s on a warm cache.
